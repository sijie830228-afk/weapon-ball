        ...
        function App() {
          ...
          const startGame = () => {
            ...
            const engine = {
              ...
              spawnProjectile: p => { if(engine.hasPrometheus) engine.dischargeCount=(engine.dischargeCount||0)+1; soundRef.current.playProjectile(p); engine.projectiles.push(p); }, spawnWave: w => { if(engine.hasPrometheus) engine.dischargeCount=(engine.dischargeCount||0)+1; w.currentRadius=w.startRadius||0; w.hitSet=new Set(); w.lingerTimer=0; soundRef.current.playWave(w); engine.waves.push(w); }, spawnParticle: p => engine.particles.push({...p,lifespan:p.maxLifespan||1.5,maxLifespan:p.maxLifespan||1.5}), spawnObstacle: o => { if(engine.hasPrometheus) engine.dischargeCount=(engine.dischargeCount||0)+1; soundRef.current.playObstacle(o); engine.obstacles.push(o); },
              applyHeal: (t, a, src, allowOverheal = false) => {
                 if(t.hp<=0||t.isBlank)return;
                 const canOverheal = allowOverheal || engine.balls.some(x => x.team === t.team && x.hp > 0 && (x.id === 'ecmo' || x.copied === 'ecmo'));
              ...
              engine.hasPrometheus = engine.balls.some(b=>b.id==='prometheus0'||b.copied==='prometheus0');
              engine.dischargeCount = 0;
              },
              update: () => {
                engine.time+=DT;
          ...
          useEffect(() => {
            ...
            const render = cT => {
              ...
              eng.projectiles.forEach(p=>{ctx.beginPath();
                ...
                else if(p.type==='heartfire'){ctx.beginPath();ctx.arc(p.x,p.y,p.radius+5+sin(eng.time*10)*2,0,PI*2);ctx.fillStyle='rgba(253,186,116,0.25)';ctx.fill();ctx.beginPath();ctx.moveTo(p.x,p.y+p.radius*.55);ctx.bezierCurveTo(p.x-p.radius*1.2,p.y-p.radius*.2,p.x-p.radius*.55,p.y-p.radius*1.05,p.x,p.y-p.radius*.35);ctx.bezierCurveTo(p.x+p.radius*.55,p.y-p.radius*1.05,p.x+p.radius*1.2,p.y-p.radius*.2,p.x,p.y+p.radius*.55);ctx.fillStyle=p.color;ctx.fill();ctx.strokeStyle='#FFF7ED';ctx.lineWidth=1.5;ctx.stroke();}
                ...
                else if(p.type==='n_dart'){const a=atan2(p.vy,p.vx);ctx.save();ctx.translate(p.x,p.y);ctx.rotate(a);/* barrel */ctx.beginPath();ctx.moveTo(16,0);ctx.lineTo(9,3);ctx.lineTo(-5,3.5);ctx.lineTo(-5,-3.5);ctx.lineTo(9,-3);ctx.closePath();ctx.fillStyle=p.color;ctx.fill();/* silver tip */ctx.beginPath();ctx.moveTo(16,0);ctx.lineTo(9,2.5);ctx.lineTo(9,-2.5);ctx.closePath();ctx.fillStyle='#F5D0FE';ctx.fill();/* top fin */ctx.beginPath();ctx.moveTo(-5,3);ctx.lineTo(-16,9);ctx.lineTo(-13,3);ctx.closePath();ctx.fillStyle=p.color;ctx.fill();/* bottom fin */ctx.beginPath();ctx.moveTo(-5,-3);ctx.lineTo(-16,-9);ctx.lineTo(-13,-3);ctx.closePath();ctx.fillStyle=p.color;ctx.fill();ctx.restore();}
              ...
              eng.balls.forEach(b=>{if(b.hp<=0)return; const aS=new Set((b.statuses||[]).map(s=>s.type));
                ...
                if(b.id==='companion'||b.copied==='companion'){const disabled=b.cannonDisabledTimer>0, rr=b.radius+30;ctx.beginPath();ctx.arc(b.x,b.y,rr,0,PI*2);ctx.strokeStyle=disabled?'rgba(148,163,184,0.25)':'rgba(103,232,249,0.22)';ctx.lineWidth=1.5;ctx.stroke();for(let i=0;i<3;i++){const a=(b.cannonAngle||0)+i*PI*2/3,cx=b.x+cos(a)*rr,cy=b.y+sin(a)*rr;ctx.save();ctx.translate(cx,cy);ctx.rotate(a+PI/2);ctx.beginPath();ctx.roundRect?ctx.roundRect(-7,-11,14,22,4):ctx.rect(-7,-11,14,22);ctx.fillStyle=disabled?'#475569':'#164E63';ctx.fill();ctx.strokeStyle=disabled?'#94A3B8':'#67E8F9';ctx.lineWidth=2;ctx.stroke();ctx.restore();}if(b.mechTimer>0){ctx.beginPath();ctx.arc(b.x,b.y,b.radius+12,0,PI*2);ctx.strokeStyle='rgba(103,232,249,0.75)';ctx.lineWidth=4;ctx.setLineDash([10,6]);ctx.stroke();ctx.setLineDash([]);}}
                ...
                if(b.isCore){ctx.save();ctx.translate(b.x,b.y);const pulse=0.85+0.15*sin(eng.time*3);ctx.rotate(eng.time*0.4);ctx.beginPath();for(let i=0;i<6;i++){const a=i*PI/3,r=b.radius*pulse;const px=cos(a)*r,py=sin(a)*r;i===0?ctx.moveTo(px,py):ctx.lineTo(px,py);}ctx.closePath();ctx.fillStyle='rgba(125,211,252,0.18)';ctx.fill();ctx.strokeStyle=b.color;ctx.lineWidth=3;ctx.shadowColor=b.color;ctx.shadowBlur=14;ctx.stroke();ctx.shadowBlur=0;ctx.rotate(-eng.time*0.4);ctx.beginPath();ctx.arc(0,0,b.radius*0.42,0,PI*2);ctx.fillStyle='#FFF';ctx.fill();const dc=min(10,eng.dischargeCount||0);for(let i=0;i<dc;i++){const a=eng.time*1.5+i*(PI*2/10),r=b.radius*0.78;ctx.beginPath();ctx.arc(cos(a)*r,sin(a)*r,3,0,PI*2);ctx.fillStyle='#7DD3FC';ctx.fill();}ctx.restore();}
                else if(b.isChessPiece){ctx.beginPath();ctx.arc(b.x,b.y,b.radius,0,PI*2);ctx.fillStyle=b.behavior==='protect'?'rgba(56,189,248,0.15)':'rgba(248,113,113,0.15)';ctx.fill();ctx.strokeStyle=b.behavior==='protect'?'#38BDF8':'#F87171';ctx.lineWidth=2;ctx.stroke();ctx.fillStyle=b.color;ctx.font='24px sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText({'pawn':'♟','knight':'♞','bishop':'♝','rook':'♜','queen':'♛'}[b.pieceType]||'♟',b.x,b.y);}
